...
def reset_mlflow_state():
    """Reset any active MLflow runs before starting a new one."""
    try:
...
def generate_stories(input_csv: str, output_file: str) -> list[str]:
    ...
    """
    # Reset any active MLflow runs before setup
    reset_mlflow_state()
    ...
    # Use context manager to ensure run is always closed
    with mlflow.start_run(run_name="Storytelling") as run:
        stories = []
        try:
    ...
    """
            ...
            logger.info(
                f"Total: {(pd.Timestamp.now() - start_time).total_seconds():.3f}s",
            )

        except Exception as e:
            logger.exception(f"Error generating stories: {e}")
            mlflow.log_param("error", str(e))

    # Run is automatically ended by the context manager
    return stories
